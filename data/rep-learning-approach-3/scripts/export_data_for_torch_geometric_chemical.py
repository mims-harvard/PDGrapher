...
outdir = '../processed/chemical/{}'.format(binarization)
...
def main():  
    ...
    data_root_dir = '../../lincs/processed/chemical/nofilter_dose_timepoint/{}'.format(binarization)
    data_root_dir_healthy = '../../lincs/processed/chemical/{}'.format(binarization)
    # , ('RWPE1', 'ctl_vector')
    for cell_line, healthy in zip(['A549', 'MCF7'], [('NL20', 'ctl_vehicle'), ('MCF10A', 'ctl_vehicle')]):
        log_handle.write('----------------\n\nCELL LINE:{}\n----------------\n'.format(cell_line))
        #PPI
        ppi = load_ppi('../../ppi/processed/ppi_all_genes_edgelist.txt', log_handle)
        #gene info
        gene_info, dict_entrez_symbol, dict_symbol_entrez = load_gene_metadata('../../lincs/processed/chemical/nofilter_dose_timepoint/gene_info.txt', log_handle)
        #FORWARD DATA
            #healthy GE data
