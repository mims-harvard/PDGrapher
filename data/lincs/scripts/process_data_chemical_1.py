...
outdir = '../processed/chemical'
os.makedirs(outdir, exist_ok=True)
outdir_df = '../processed/chemical/dataframes/'
...
def load_data(DATA_ROOT,log_handle):
	...
	# drugProt_df=pd.read_csv("../../drugbank/processed/targets.txt")
	...
	###Load DrugBank Data
	DATAROOT= '../../drugbank/2022-11-DrugBank/data/'
	...
	# #Targets
	# row['targets_name'] = [target.findtext(n + "name") for target in drug.findall("{ns}targets/{ns}target".format(ns = ns))]
	# row['targets_uniprot_id'] = [target.find(ns + "polypeptide").get('id') for target in drug.findall("{ns}targets/{ns}target".format(ns = ns))]
	# row['targets_gene_name'] = [target.find(ns + "polypeptide").findtext(ns  + "gene-name") for target in drug.findall("{ns}targets/{ns}target".format(ns = ns))]
...
def main():
	...
	df_targets = load_targets_drugbank('../../drugbank/processed/targets.txt')
