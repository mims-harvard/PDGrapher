...
def loads_data(DATA_ROOT, log_handle):
	...
	with open('../processed/chemical/dataframes/df_targets.pickle', 'rb') as f:
		df_targets = pickle.load(f)
	

	
	drugbank_targets=pd.read_csv("../../drugbank/processed/targets.txt")
		...
		if pert_id in dict_pert_id:
			...
			if type(gene_symbols)==list:
				for j in range(len(gene_symbols)): 
					#if type(gene_symbols[j])==list:
						#sublist=gene_symbols[j]
						#for x in range(len(sublist)):
							#if sublist[x] not in dict_symbol_id: 
								#sublist[x]=None
						#sublist=list(filter(lambda item: item is not None, sublist))
						#assert all(elem in dict_symbol_id for elem in sublist)
						#if len(sublist)>0:
							#gene_symbols[j]=sublist[0] #Just picking the first one because all gene_names are synonymous in sublist

						#else: 
							#gene_symbols[j]='No symbol in dict_symbol_id'
		...
		if pert_id in dict_pert_id:
			...
			if type(gene_symbols)==list:
				for z in range(len(gene_symbols)):
					#if type(gene_symbols[z])==list:
						#sublist=gene_symbols[z]
						#for a in range(len(sublist)): 
							#if sublist[a] not in dict_symbol_id: 
								#sublist[a]=None
						#sublist=list(filter(lambda item: item is not None, sublist))
						#assert all(elem in dict_symbol_id for elem in sublist)
						#if len(sublist)>0:
							#gene_symbols[z]=sublist[0] #Just picking the first one because all gene_names are synonymous in sublist

						#else: 
							#gene_symbols[z]='No symbol in dict_symbol_id'
...
outdir = '../processed/chemical/nofilter_dose_timepoint'
os.makedirs(outdir, exist_ok=True)


def main():  
	# DATA_ROOT = "../../lincs/2022-02-LINCS_Level3/data/"
	# log_handle = open(osp.join(outdir, 'log_process_data_lognorm.txt'), 'w')
	...
	# # inst_info_cp, matrix_cp, inst_info_ctl, matrix_ctl = filter_dosage_timepoints(sample_ids_cp, inst_info_cp, matrix_cp, inst_info_ctl, matrix_ctl, log_handle)
	# # print('filtered dosages/timepoints - lognorm', inst_info_cp.shape)
	...
	# use_log=True
	# binarize_genewise_comparing_to_control(inst_info_cp, matrix_cp, inst_info_ctl, matrix_ctl, gene_info, keep_cell_lines, log_handle, outdir, use_log)
	# print('binarize_genewise_comparing_to_control - lognorm')
	# log_handle.close()
