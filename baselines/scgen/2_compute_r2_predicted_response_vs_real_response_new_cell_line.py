...
def main():
	...
	parser.add_argument('--gpu_index', default="0", type=str, help='GPU index')
	parser.add_argument('--eval_setting', default="new_cell_line", type=str, help='Evaluation setting')
	...
	for split_index in splits.keys():
		indir = 'results_lincs/{}/{}/split_{}'.format(args.dataset_type, test_cell_type, split_index)
				...
				#Get subset of data to predict for
		test_diseased = train[(train.obs["cell_type"] == test_cell_type) & (train.obs["condition"] == 'control')]
		test_treated = train[(train.obs["cell_type"] == test_cell_type) & (train.obs["condition"] != 'control')]


		#Loads data for scgen (needs to be the same subset that it was trained with)
		...
		train_data_path = osp.join(indir, 'train_tmp.h5ad')
		train_subset.write(train_data_path)
		test_diseased_path = osp.join(indir, 'test_diseased_tmp.h5ad')
		...
		perturbagens_in_test = test_treated.obs['condition'].unique().tolist()
		

		perturbagens = list(set(perturbagens_in_train).intersection(set(perturbagens_in_test)))
		perturbagens.sort()
		log = open(osp.join(indir, 'log_2_compute_r2_predicted_response_new_cell_line.txt'), 'w')	
		jj = 1
		for perturbagen in perturbagens:
			...
			# tic = time()
			# corrs_cpu = np_pearson_cor(pred_all_samples.transpose(), test_treated.X.transpose())
			# corrs_cpu = np.diag(corrs_cpu)**2
			# toc = time()
			# print('Time:{}secs'.format(toc-tic))
		...
		log.close()
		with open(osp.join(indir, 'int_disc_selected_predicted_response_new_cell_line.pickle'), 'wb') as f:
			pickle.dump(vectors_predicted_response, f)
		
		with open(osp.join(indir, 'int_disc_selected_predicted_perturbagen_new_cell_line.pickle'), 'wb') as f:
			pickle.dump(vectors_predicted_perturbagen, f)

		with open(osp.join(indir, 'int_disc_highest_r2_predicted_response_vs_real_response_new_cell_line.pickle'), 'wb') as f:
