# This code is a modified version of torch_geometric.nn.conv.message_passing.py
# from PyTorch Geometric 2.0.4. Original code is available here:
# https://pytorch-geometric.readthedocs.io/en/2.0.4/_modules/torch_geometric/nn/conv/message_passing.html#MessagePassing
...
class MessagePassing(torch.nn.Module):
    ...
    #@guada: changed to collect messages for all samples using a single edge_index (added batch_size as arg)
    def __lift__(self, src, edge_index, dim, batch_size, kwargs):
        #@guada: this is the function that builds the messages 
        
        if isinstance(edge_index, Tensor):
            index = edge_index[dim]
            
            node_dim = 1
            out = src.view([batch_size, -1] + [e for e in src.shape[1:]]).index_select(node_dim, index).view([-1] +  [e for e in src.shape[1:]]) #@guada: changed to collect messages for all samples using a single edge_index
        ...
        for arg in args:
            if arg[-2:] not in ['_i', '_j']:
                ...
                if isinstance(data, Tensor):
                    ...
                    data = self.__lift__(data, edge_index, dim, batch_size, kwargs)
    ...
    #@guada: modified so that I aggregate messages for each graph using index from 1 single graph
    def aggregate(self, inputs: Tensor, index: Tensor,
        ...
        if ptr is not None:
            ...
            return scatter(inputs.view([batch_size, -1] +  [e for e in inputs.shape[1:]]) , index, dim=node_dim, dim_size=int(dim_size/batch_size), reduce=self.aggr).view([-1] +  [e for e in inputs.shape[1:]])
